...
const STORAGE_KEY = 'feedback-store-v1'
...
const loadFeedbackState = (): { feedbacks: Feedback[]; repairOrders: RepairOrder[] } => {
  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    if (raw) {
      const data = JSON.parse(raw)
      if (data.feedbacks && data.repairOrders) {
        return { feedbacks: data.feedbacks, repairOrders: data.repairOrders }
      }
    }
  } catch {}
  return { feedbacks: [...initialFeedbacks], repairOrders: [...initialRepairOrders] }
}
...
export const useFeedbackStore = create<FeedbackState>((set, get) => {
  const initial = loadFeedbackState()
  return {
    feedbacks: initial.feedbacks,
    repairOrders: initial.repairOrders,
...
useFeedbackStore.subscribe((state) => {
  try {
